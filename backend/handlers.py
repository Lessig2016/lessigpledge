...
PLEDGE_SCHEMA = dict(
    ...
    properties=dict(
        ...
        recurrence_period=dict(type='string', required=False,
                               enum=valid_recurrence_periods),
...
def pledge_helper(handler, data, stripe_customer_id, stripe_charge_id, paypal_payer_id, paypal_txn_id):
  ...
  if amountCents >= 100000:
    ...
    lessig_body = open(
        'email/lessig-notify.txt').read().format(**format_kwargs)
...
class PledgeHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    if stripe_customer_id:
      try:
        env.stripe_backend.UpsellCustomerToMonthlySubscription(
            stripe_customer_id, data['amountCents'] / 100)
      ...
      try:
        if data.get('recurring', '') == True:
          logging.info(
              'Trying to create stripe customer %s for a recurring donation' % data['email'])
          ...
          logging.info(
              'Trying to create stripe customer %s for a single donation' % data['email'])
        ...
        if len(stripe_customer.sources.data) > 0:
          ...
          if 'address_line1_check' in card_data:
            logging.info('Address check: %s' %
                         card_data['address_line1_check'])
            if card_data['address_line1_check'] == 'fail':
              ...
              # return  # error trapping is not working in here, so have to do
              # hacky early return for now
          ...
          stripe_charge_id = env.stripe_backend.Charge(
              stripe_customer_id, data['amountCents'])
    ...
    id, auth_token, uut, receipt_url = pledge_helper(
        self, data, stripe_customer_id, stripe_charge_id, None, None)
...
class SubscribeHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    if volunteer_input == 'Yes':
      ...
      text_body = open(
          'email/volunteer-thank-you.txt').read().format(**format_kwargs)
      html_body = open(
          'email/volunteer-thank-you.html').read().format(**format_kwargs)
    ...
    rootstrikers_input = cgi.escape(self.request.get(
        'rootstrikers'))  # Free text, limited to 255 char
...
class ThankTeamHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    # if only sending to new members, filter out those that have already
    # received emails
...
class BitcoinNotificationsHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    logging.info('Bitpay notifications for. Invoice ID: %s, Status: %s' %
                 (invoiceID, data.get('status')))
    ...
    if not data.get('status') == 'confirmed':
      logging.info('Non comfirmed transaction. Ignoring. Invoice ID: %s, Status: %s' % (
          invoiceID, data.get('status')))
...
# Paypal Step 2: Paypal returns to us, telling us the user has agreed.
# Book it.
class PaypalReturnHandler(webapp2.RequestHandler):
  ...
  def get(self):
    ...
    if not token or not payer_id:
      ...
      self.error(400)
      self.response.write(
          "Unusual error: no token or payer id from Paypal.  Please contact info@lessig2016.us and report these details:")
    ...
    if not rc:
      self.error(400)
      self.response.write(
          "Unusual error: Could not get payment details from Paypal.  Please contact info@lessig2016.us and report these details:")
    ...
    if rc:
      ...
      self.error(400)
      self.response.write(
          "Unusual error: Could not get complete payment from Paypal.  Please contact info@lessig2016.us and report these details:")
      self.response.write(pprint.pformat(results))
      return
...
class IssuePollingHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    text_body = open(
        'email/issue-survey-response.txt').read().format(**format_kwargs)
    html_body = open(
        'email/issue-survey-response.html').read().format(**format_kwargs)
...
class CandidatePollingHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    text_body = open(
        'email/voting-thank-you.txt').read().format(**format_kwargs)
    html_body = open(
        'email/voting-thank-you.html').read().format(**format_kwargs)
